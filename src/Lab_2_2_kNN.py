...
class knn:
    ...
    def predict(self, X: np.ndarray) -> np.ndarray:
        ...
        if self.x_train is None or self.y_train is None:
            raise ValueError("Model has not been fitted. Call 'fit' before predicting.")
...
def plot_calibration_curve(y_true, y_probs, positive_label, n_bins=10):
    ...
    true_proportions = [
        np.mean(y_true_mapped[bin_indices == i]) if np.sum(bin_indices == i) > 0 else 0
...
def plot_roc_curve(y_true, y_probs, positive_label):
    ...
    total_positives = np.sum(y_true == positive_label)
